...
class AddSamplesZero(ename2:String = "", logN: Int, minD: Int, maxD: Int, qd: Int)(implicit timestampedfolder:String) extends Experiment (s"ubuntuone", ename2, "ubuntuone-expts"){
    val domain = 4
    ...
    (dc: DataCube, dcname: String, qu: IndexedSeq[Int], trueResult: Array[Double]) = {
        ...
        (0 to numGroups).foreach { i =>
            (0 until 1 << groupSize).foreach { j =>
             ...
             if (i % ((1).max(numGroups / domain)) == 0) {
                    ...
                    val allRatio_Hybrid = (0 until result_len_Hybrid).map(i => if (trueResultAll_Hybrid(i)==0) errorAll_Hybrid(i) / 0.00000001 else errorAll_Hybrid(i) / trueResultAll_Hybrid(i))
                    val maxRatio_Hybrid = allRatio_Hybrid.max
                    val avgRatio_Hybrid = allRatio_Hybrid.foldLeft(0.0)(_ + _) / allRatio_Hybrid.length
                    val allMult_Hybrid = (0 until result_len_Hybrid).map(i => (if(trueResultAll_Hybrid(i)==0) hybrid_result(i)/ 0.00000001 else hybrid_result(i) / trueResultAll_Hybrid(i)).max(if(hybrid_result(i)==0) trueResultAll_Hybrid(i) / 0.00000001 else trueResultAll_Hybrid(i) / hybrid_result(i)))
                    ...
                    val allRatio_Naive = (0 until result_len_Naive).map(i => if(trueResultAll_Naive(i)==0) errorAll_Naive(i) / 0.00000001 else errorAll_Naive(i) / trueResultAll_Naive(i))
                    val maxRatio_Naive = allRatio_Naive.max
                    val avgRatio_Naive = allRatio_Naive.foldLeft(0.0)(_ + _) / allRatio_Naive.length
                    val allMult_Naive = (0 until result_len_Naive).map(i => (if(trueResultAll_Naive(i)==0) naive_result(i) / 0.00000001 else naive_result(i) / trueResultAll_Naive(i)).max(if(naive_result(i)==0) trueResultAll_Naive(i) / 0.00000001 else trueResultAll_Naive(i) / naive_result(i)))
        ...
        val allRatio_Moment = (0 until result_len_Moment).map(i => if(trueResultAll_Moment(i)==0) errorAll_Moment(i) / 0.00000001 else errorAll_Moment(i) / trueResultAll_Moment(i))
...
object AddSamplesZero extends ExperimentRunner {
    def qsize(cg: CubeGenerator, isSMS:Boolean)(implicit timestampedFolder: String, numIters: Int, be: CBackend) = {
        ...
        val (logN, minD, maxD, qd) = (12,15,30,10)
        ...
        val expt = new AddSamplesZero(ename, logN, minD, maxD, qd)
