...
class UbuntuOneExperiment(ename2:String = "")(implicit timestampedfolder:String) extends Experiment (s"zipf", ename2, "zipf-expts"){
    val domain = 1
...
object UbuntuOneExperiment extends ExperimentRunner {
    def qsize(cg: CubeGenerator)(implicit timestampedFolder: String, numIters: Int, be: CBackend) = {
        ...
        val expt = new UbuntuOneExperiment(ename)

        val dc = cg.loadSMS(logN, minD, maxD)
        dc.loadPrimaryMoments(cg.baseName)

            val query = Vector(0, 1, 20, 21, 30, 31)
