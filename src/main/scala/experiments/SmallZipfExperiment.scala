...
class SmallZipfExperiment(ename2:String = "")(implicit timestampedfolder:String) extends Experiment (s"zipf", ename2, "zipf-expts"){
    val domain = 1
...
object SmallZipfExperiment extends ExperimentRunner {
    def qsize(cg: CubeGenerator)(implicit timestampedFolder: String, numIters: Int, be: CBackend) = {
        ...
        val expt = new SmallZipfExperiment(ename)

        val dc = cg.loadSMS(logN, minD, maxD)
        dc.loadPrimaryMoments(cg.baseName)

            val query = Vector(0,1)
