...
class TestExperiment(ename2:String = "")(implicit timestampedfolder:String) extends Experiment (s"zipf", ename2, "zipf-expts"){
    ...
    (dc: DataCube, dcname: String, qu: IndexedSeq[Int], trueResult: Array[Double]) = {
        ...
        val allPeter_Moment = (0 until result_len_Moment).map(i => trueResultAll_Moment(i) / errorAll_Moment(i))
        ...
        (0 to numGroups).foreach { i =>
            (0 until 1 << groupSize).foreach { j =>
             ...
             if (i % (numGroups / 3) == 0) {
                    ...
                    val allRatio_Hybrid = (0 until result_len_Hybrid).map(i => errorAll_Hybrid(i) / trueResultAll_Hybrid(i))
                    val maxRatio_Hybrid = allRatio_Hybrid.max
                    hybrid_errors = hybrid_errors :+ maxRatio_Hybrid
                    val allPeter_Hybrid = (0 until result_len_Hybrid).map(i => trueResultAll_Hybrid(i) / errorAll_Hybrid(i))
                    ...
                    val allRatio_Naive = (0 until result_len_Naive).map(i => errorAll_Naive(i) / trueResultAll_Naive(i))
                    val maxRatio_Naive = allRatio_Naive.max
                    naive_errors = naive_errors :+ maxRatio_Naive
                    val allPeter_Naive = (0 until result_len_Naive).map(i => trueResultAll_Naive(i) / errorAll_Naive(i))
...
object TestExperiment extends ExperimentRunner {
    def qsize(cg: CubeGenerator, isSMS:Boolean)(implicit timestampedFolder: String, numIters: Int, be: CBackend) = {
        ...
        val expt = new TestExperiment(ename)
    ...
    def main(args: Array[String]): Unit = {
        implicit val be = CBackend.default
        val zipf = new ZipfGenerator2(1.8)
